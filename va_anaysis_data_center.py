...
def pickle_data_analysis_center_2(args2):
    ...
    idx_d = 0
    for d in data:
        pi_via_mdp = d['pi_via_mdp']
        Q_via_mdp = d['Q_via_mdp']
        V_via_mdp = np.einsum('ij,ji->j', Q_via_mdp, pi_via_mdp)
        V = d['Q'].max(axis=0)
        V_del = np.abs(V_via_mdp - V)
        B = d['B']
        B_pi_via_mdp = np.einsum('ij,ji->j', B, pi_via_mdp)
        va_states = d['va_states']
        phi = d['phi']
        v_y = []
        for x in va_states:
            mask = [y == x for y in phi]
            del_x = (B_pi_via_mdp * V_del * mask).sum() / (B_pi_via_mdp * mask).sum()
            v_y = np.hstack((v_y, del_x))

        # num_a, num_s, _ = d['T'].shape
        # num_x = len(d['va_states'])
        # v_del = np.abs(d['Q'].max(axis=0) - d['Q_via_mdp'].max(axis=0))
        # q_y = np.abs(d['Q'] - d['Q_via_mdp'])
        # q_x = d['B']
        # idx = d['Q'].argmax(axis=0)
        # v_y = np.abs(d['Q'].max(axis=0) - d['Q_via_mdp'].max(axis=0))
        ...
        #     s += 1
        # v_x = mdps.stationary_dist_eigenvalue(d['T'], args, d['pi_via_mdp'])
        v_plt.scatter(range(len(v_y)), v_y , s=msize)
        # v_log_plt.scatter(v_x, v_y, s=2)
        # q_plt.scatter(q_x, q_y, s=msize)
        # q_log_plt.scatter(q_x, q_y, s=2)
        # q_norm_plt.scatter(num_s + num_a, q_y.max()-q_y.min(), s=msize)
...
pickle_data_analysis_center_2(args)